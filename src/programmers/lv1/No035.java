...
public class No035 {
    public int solution(String t, String p) {
        int answer = 0;
        Long[] numbers = new Long[t.length()-(p.length()-1)];

        for (int i = 0; i < numbers.length; i++) {
            numbers[i] = Long.parseLong(t.substring(i,i+p.length()));
            if (numbers[i] <= Long.parseLong(p)) {
                answer++;
            }
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(no035.solution("3141592","271"));
    }
}
